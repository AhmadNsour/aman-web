...
function TrustCard({ title, description, icon }: { title: string; description: string; icon: React.ReactNode }) {
  return (
    <div className="flex gap-3 rounded-2xl border bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <div className="grid h-10 w-10 flex-none place-items-center rounded-xl bg-purple-100 text-purple-600">{icon}</div>
  ...
  return (
    <aside className="hidden space-y-4 lg:block">
      <div className="rounded-3xl border bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm leading-relaxed text-gray-600">Safety is not a feature — it is the foundation. Every step of your application is designed with your peace of mind in mind.</p>
      </div>
